...
class helper_format{
	...
	*/
	...
	public static function dataPost($dataCompleta){
		if (!empty($dataCompleta)){
           $dataCompleta = explode(" ", $dataCompleta);
           $data = helper_format::dataEngToBr($dataCompleta[0]);
           return $data." ".$dataCompleta[1];
       }
	}
	...
	*/
	...
	public static function muda_data($dt){
		$dia = substr($dt,0,2);
		$mes = substr($dt,3,2);
		$ano = substr($dt,6,4);
		$data = $ano."/".$mes."/".$dia;
